...
class Upload {
  ...
  _emitCreated(location, response) {
    if (typeof this.options.onCreated === "function") {
      const options = this.options.onCreated(location, response, this.options);
      if (options) {
  ...
  _emitResumed(uploadOffset) {
    if (typeof this.options.onResumed === "function") {
      const options = this.options.onResumed(this.url, uploadOffset, this.options);
      if (options) {
        this.options = options;
      }
    }
  ...
  _createUpload() {
    ...
    xhr.onload = () => {
      ...
      this._emitCreated(location, xhr.response || xhr.responseText);
